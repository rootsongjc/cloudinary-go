...
func TestCleanExtensionNameWithPrepend(t *testing.T) {
	assets := [][4]string{
		// order: path, prepend, expected result
		{"/tmp/css/default.css", "new", "new/default"},
		{"/a/b/c.png", "", "c"},
		{"/a/b/c.png", "  ", "c"}, // With spaces
		{"/a/b/c.png", "/x", "x/c"},
		{"c.png", "/x", "x/c"},
	}
	for _, p := range assets {
		c := CleanExtensionNameWithPrepend(p[0], p[1])
		if c != p[2] {
			t.Errorf("wrong cleaned name. Expect '%s', got '%s'", p[2], c)
		}
	}
}
